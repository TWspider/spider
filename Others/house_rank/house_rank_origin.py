...
class RankHanle(object):
    def __init__(self):
        self.engine_third_house = create_engine('mssql+pymssql://bigdata:pUb6Qfv7BFxl@10.10.202.13/TWSpider')
        self.engine_tw_house = create_engine('mssql+pymssql://bigdata:pUb6Qfv7BFxl@10.10.202.13/TWEstate')
        self.engine_res = create_engine(
            'mssql+pyodbc://{0}:{1}@{2}?driver=SQL Server Native Client 11.0'.format("bigdata", "pUb6Qfv7BFxl",
                                                                                     "10.10.202.13/TWSpider"),
    ...
    def rank_handle_inner(self, res_match_is, res_match_is_sql, rank_tw, rank_third, rank, field_list):
        ...
        if not res_match_not.empty:
            res_match_not.loc[:, 'star_level'] = rank
...
class MatchRank:
    ...
    def __init__(self, flag_all=False):
        ...
        if flag_all:
            ...
            self.sql_str_day = "select DISTINCT RoomId as id_third,PropertyAddress,PropertyCommunity,TotalFloor,Floor,HouseType,BuildingSquare,HouseDesc,PriceUnit from ThirdHouseResource WHERE HouseStatus='可售' or HouseStatus='可租'"
        else:
    ...
    def match_rank(self):
        house_third_list = pd.read_sql(
            self.sql_str_day,
            self.engine_third_house, chunksize=200000)
        res_match_is_sql = pd.DataFrame(columns=["id_third", "id_tw", "star_level", "input_time"])
        res_match_is_sql.to_sql('house_rank_new', con=self.rh.engine_res, if_exists="replace", index=False,
                                dtype=self.rh.dtype)
        for house_third in house_third_list:
            if not house_third.empty:
                ...
                for house_tw in house_list_tw:
                    ...
                    res_match_is_sql.to_sql('house_rank_new', con=self.rh.engine_res, if_exists="append", index=False,
                                            dtype=self.rh.dtype)
                ...
                if id_third_match_not:
                    ...
                    res_match_not_sql.loc[:, 'star_level'] = 0
                    res_match_not_sql.loc[:, 'input_time'] = input_time
                    res_match_not_sql.loc[:, 'id_tw'] = None
                    res_match_not_sql.to_sql('house_rank_new', con=self.rh.engine_res, if_exists="append", index=False,
                                             dtype=self.rh.dtype)
            else:
                print("无新增数据")
...
if __name__ == '__main__':
    ...
    # 判断总量还是增量flag_all
    mr = MatchRank(flag_all=True)
    # print(house_third)
    # 处理第三方房源
    mr.match_rank()
    with pymssql.connect(host='10.10.202.13', database='TWSpider',
                         user='bigdata', password='pUb6Qfv7BFxl', charset="utf8",) as conn:
        with conn.cursor() as cursor:
            # 删除house_rank
            try:
                cursor.execute("Drop table if exists house_rank")
                # 更名house_rank_new 为 house_rank
                cursor.execute("EXEC sp_rename 'house_rank_new', 'house_rank'")
                conn.commit()
            except:
                print("无此数据表")
    time_end = time.time()
